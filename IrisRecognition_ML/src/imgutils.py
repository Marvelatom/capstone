...
def findTopEyelid(imsz, imageiris, irl, icl, rowp, rp, ret_top=None):
    ...
    if lines.size > 0:
        ...
        # Ensure yl and xl are within mask bounds
        yl = np.clip(yl, 0, imsz[0] - 1)  # clip values to valid y range
        xl = np.clip(xl, 0, imsz[1] - 1)  # clip values to valid x range

        yla = np.max(yl)
        y2 = np.arange(yla)

        # Apply mask for valid coordinates
        mask[yl, xl] = np.nan

        # Create grid for further masking
        grid = np.meshgrid(y2, xl)

        # Ensure grid values are within bounds before applying to mask
        grid_y = np.clip(grid[0], 0, imsz[0] - 1)
        grid_x = np.clip(grid[1], 0, imsz[1] - 1)
        mask[grid_y, grid_x] = np.nan
...
def findBottomEyelid(imsz, imageiris, rowp, rp, irl, icl, ret_bot=None):
    ...
    if lines.size > 0:
        ...
        # Clip yl and xl to be within valid mask boundaries
        yl = np.clip(yl, 0, imsz[0] - 1)  # Ensure yl stays within the row limits
        xl = np.clip(xl, 0, imsz[1] - 1)  # Ensure xl stays within the column limits

        yla = np.min(yl)
        y2 = np.arange(yla - 1, imsz[0])

        # Apply mask only within valid indices
        valid_indices = (yl >= 0) & (yl < imsz[0]) & (xl >= 0) & (xl < imsz[1])
        mask[yl[valid_indices], xl[valid_indices]] = np.nan

        # Generate grid and ensure it's within bounds
        grid_y, grid_x = np.meshgrid(y2, xl)
        grid_y = np.clip(grid_y, 0, imsz[0] - 1)
        grid_x = np.clip(grid_x, 0, imsz[1] - 1)
